...
class SVD_GCN(nn.Module):
    def __init__(self, user_size, item_size, beta=2.0, coef_u=0.5, coef_i=0.7, req_vec=60, latent_size=64, reg=0.01):
        ...
        self.FS = Variable(
            torch.nn.init.uniform_(torch.randn(req_vec, latent_size), -np.sqrt(6. / (req_vec + latent_size)),
                                   np.sqrt(6. / (req_vec + latent_size))).cuda(), requires_grad=True)
    ...
    def forward(self, u, p, n, up, un, pp, pn):
        ...
        final_user, final_pos, final_nega = self.user_vector[u].mm(self.FS), self.item_vector[p].mm(self.FS), \
                                            self.item_vector[n].mm(self.FS)
        ...
        self_loss_u = torch.log(
            ((final_user * final_user_p).sum(1) - (final_user * final_user_n).sum(1)).sigmoid()).sum()
    ...
    def test(self):
        ...
        ndcg10, ndcg20, recall10, recall20 = round(ndcg10 / user, 4), round(ndcg20 / user, 4), round(recall10 / user,
                                                                                                     4), round(
            recall20 / user, 4)
